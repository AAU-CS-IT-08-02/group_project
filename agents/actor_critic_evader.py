...
class ActorCriticEvader(BaseAgent):
    """
    Actor-Critic evader.
    Learns to reach the goal while avoiding capture.
    Exploration decays to 1%.
    """
    ...
    def __init__(self, role: str, config: dict):
        ...
        # Exploration schedule for evader
        self.start_explore = 0.03
    ...
    def _current_explore_prob(self) -> float:
        return max(
            self.min_explore,
            self.start_explore * (self.explore_decay ** self.step_counter)
        )
    ...
    def select_action(self, observation: dict) -> int:
        ...
        with torch.no_grad():
            ...
            if self.training:
                explore_prob = self._current_explore_prob()
                self.step_counter += 1
    ...
    def get_reward(
        ...
        # Reward moving away from pursuer
