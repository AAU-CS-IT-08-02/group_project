...
class ActorCriticPursuer(BaseAgent):
    """
    The runner calls select_action() and update() every step.
    """
    ...
    def _obs_to_vector(self, observation: dict) -> np.ndarray:
        ...
        evader_goal_dist = abs(orow - gr) + abs(ocol - gc)
    ...
    def select_action(self, observation: dict) -> int:
        ...
        with torch.no_grad():
            ...
            if self.training:
                explore_prob = max(0.02, 0.20 * (0.999 ** getattr(self, "step_counter", 0)))
    ...
    def get_reward(
        ...
        """
        Custom reward matching the tuned Actor-Critic setup.
        """
        if role != "pursuer":
        ...
        reward += 1.8 * float(prev_agent_dist - curr_agent_dist)
        reward -= 0.8 * float(prev_evader_goal_dist - curr_evader_goal_dist)
...
AGENT_CLASS = ActorCriticPursuer